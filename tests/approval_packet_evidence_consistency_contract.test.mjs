...
function validate(record) {
  ...
  if (/READY$/.test(record.readiness_classification) && !allChecksPass) errors.push("ready with failed consistency check");
...
test("READY classification with a failed check is blocked", () => {
  assert.ok(validate(readJson(paths.readyWithFailure)).includes("ready with failed consistency check"));
});
